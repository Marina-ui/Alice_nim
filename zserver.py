    ...
    if answer == 'обучение' and context['mode'] == 'new' or \
        ...
        if context['mode'] == 'игра':
            ...
            text = f'Правила в игре с одной кучкой просты: задаётся начальное количество камней в куче. ' \
                   ...
                   f'В нашей куче {chips[0]} {stones}. Можно брать 1-{chips_out} камней. '
            context['chips'][0] -= motion[1]
        ...
        if kuchki == 1:
            ...
            motion = generate_motion(1, chips, chips_out=chips_out, first_motion=True)
            stones = declination(chips[0], 'камень')
            rules = f'Объясняю: всего {chips[0]} {stones} в куче. Мы по очереди берём 1-{chips_out} камней.' \
        ...
        if kuchki == 1:
          ...
          context['mode'] == 'обучение' and context['first_motion']) and \
            ('забираю последн' in answer or 'беру последн' in answer or 'беру вс' in answer or
             req["request"]["nlu"]["tokens"][0].lower() in ['беру', 'взял', 'забираю', 'взяла'] or
        ...
        elif user_motion[0].lower() in ['беру', 'взял', 'забираю', 'взяла'] and \
                user_motion[1] in answ_let_to_num.keys():                    #I FIXED HERE
        ...
        if len(user_motion) == 1 and kuchki > 1:
            ...
            if ('забираю последн' in answer or 'беру последн' in answer) and chips[0] > chips_out or \
                ...
                if chips_out == 5:
                    ...
                    text += f'Вы можете вять максимум {chips_out} {stones}.'
                else:
            ...
            if (chips[0] <= 0 or 'забираю последн' in answer or 'беру последн' in answer) and \
                ...
                if context['twok_tng_finished'] and context['first_learn_game']:
                    ...
                    text += f'Вы завершили обучение с одной и двумя кучами. ' \
                            f'У вас ушло на это суммарно {efforts} {attempts}. ' \
                ...
                if not context['twok_tng_finished'] and context['first_learn_game']:
                    ...
                    text += f'Вы завершили обучение с одной кучей. ' \
                            f'У вас ушло на это всего лишь {efforts} {attempts}. ' \
            ...
            if ('забираю последн' in answer or 'беру последн' in answer or
                ...
                if context['mode'] == 'игра':
                    ...
                    context['game_finished'] = True
                    context['victories'] += 1
                    return generate_response(req, 'Забирайте, вам нужнее. Поздравляю с победой. '
                                                  'Не хотите сыграть ещё?', audio_id='W')
                elif context['mode'] == 'обучение':
                  ...
                  'беру вс' in answer or 'забираю вс' in answer) and \
                    ('из первой' in answer or 'из второй' in answer or 'из 1' in answer or 'из 2' in answer):
            ...
            elif len(user_motion) == 4 and user_motion[0].lower() in ['беру', 'взял', 'забираю', 'взяла'] \
                    and user_motion[2].lower() == 'из' and user_motion[1].isdigit():
                umotion, kuchka = int(user_motion[1]), user_motion[3]
            elif (len(user_motion) == 5 or len(user_motion) == 6 and 'куч' in user_motion[5].lower()) and \
                    user_motion[0].lower() in ['беру', 'взял', 'забираю', 'взяла'] and \
                    'камн' in user_motion[2].lower() and user_motion[3].lower() == 'из' and user_motion[1].isdigit():
                ...
                return generate_response(req, 'Неправильный формат ответа. Скажи сколько камней вы берёте, '
                                              'а затем из какой кучи. ')
            ...
            if chips[0] == 0 and chips[1] == 0:
                if context['mode'] == 'игра':
                    ...
                    if context['onek_tng_finished'] and context['first_learn_game']:
                        # предлагает сертификат о прохождение одной кучки и предлагает сыграть с ней,
                        # если у игрока уже есть сертификат об обучении с одной кучкой
                        text += f'Вы завершили обучение с одной и двумя кучами. Не хотите теперь обыграть меня?'
                        context['ask_for_play'] = True
                    if not context['onek_tng_finished'] and context['first_learn_game']:
                        # предлагает сертификат о прохождении с двумя кучками и предлагает пройти обучение с одной
                        text += f'Не хотите теперь пройти обучение с одной кучей?'
                        context['ask_for_learn'] = True
                    context['first_learn_game'] = False
                    return generate_response(req, text)
    ...
    if word == 'камень' or word == 'попытка':
        if amount == 1 or amount == 21:
            if word == 'камень':
                ...
                return 'попытка'
        elif int(str(amount)[-1]) in [2, 3, 4, 22, 23, 24]:
            if word == 'камень':
                return 'камня'
            elif word == 'попытка':
                return 'попытки'
